...
`POLICY_AUD` cannot be set before the Access application it identifies exists, so a first deployment answers `403 Access is not configured` until Access is enabled on the workers.dev URL and the secret follows. That denial is the intended state, not a fault.
