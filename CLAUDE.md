...
| Window anchor      | Both reports end on the last day every source has finished counting; `until` overrides it                     | Search Console reports that boundary only by omitting the days past it, so it is asked rather than assumed — a constant is wrong whenever Google runs late, and a period still being counted read against a complete one is a collapse that never happened. Analytics shares the anchor despite having no lag, because two reports covering different days cannot be read side by side                                                                                                                                                     |
| Shared vocabulary  | One spelling per value whichever source answered; Search Console's is translated on both the way in and out   | Two sources are only worth holding if a page or a country is one name in both. A failed crossing is silent rather than loud — Search Console answers an alpha-2 filter with no rows and no error — so it refuses instead of passing through                                                                                                                                                                                                                                                                                                |
